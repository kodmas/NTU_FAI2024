...
def standardize(X: np.ndarray) -> np.ndarray:
    ...
    # You can try other standardization methods, e.g., min-max, etc.
...
class LinearModel:
    def __init__(
        self, learning_rate=0.01, iterations=100000, model_type="linear"
    ) -> None:
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.model_type == "linear":
            ...
            # TODO: 2%
            return np.argmax(self._softmax(X @ self.weights), axis=1)

    def _compute_gradients(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        n_features = X.shape[1]
        if self.model_type == "linear":
            # TODO: 3%
            return 1/n_features * X.T @ (X @ self.weights - y)
        elif self.model_type == "logistic":
            # TODO: 3%
            # print(X @ self.weights)
            # print("softmax output",self._softmax(X @ self.weights))
            # print("true", y)
            Y_onehot = np.zeros((y.size, y.max()+1))
            Y_onehot[np.arange(y.size),y] = 1
            # print("onehot", Y_onehot)
            return 1/n_features * X.T @ (self._softmax(X @ self.weights) - Y_onehot)
...
class DecisionTree:
    ...
    def _gini_index(self, left_y: np.ndarray, right_y: np.ndarray) -> float:
        ...
        left_gini = 1 - np.sum((np.unique(left_y, return_counts=True)[1] / len(left_y))**2)
        right_gini = 1 - np.sum((np.unique(right_y, return_counts=True)[1] / len(right_y))**2)
        return len(left_y) / (len(left_y) + len(right_y)) * left_gini + len(right_y) / (len(left_y) + len(right_y)) * right_gini
    ...
    def _mse(self, left_y: np.ndarray, right_y: np.ndarray) -> float:
        ...
        return len(left_y) / (len(left_y) + len(right_y)) * left_mse + len(right_y) / (len(left_y) + len(right_y)) * right_mse
...
class RandomForest:
    def __init__(
        ...
        for i in range(n_estimators):
            self.trees.append(DecisionTree(max_depth=max_depth, model_type=model_type))
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        # TODO: 2%
        if self.model_type == "classifier":
            return np.array([Counter([tree._traverse_tree(x, tree.tree) for tree in self.trees]).most_common(1)[0][0] for x in X])
        else:
            return np.array([np.mean([tree._traverse_tree(x, tree.tree) for tree in self.trees]) for x in X])
...
def main():
    ...
    # print("Standardize Logistic Regression Accuracy:", accuracy(y_test, y_pred))
    # print('---')
    ...
    logistic_regression.fit(X_train, y_train)
    y_pred = logistic_regression.predict(X_test)
    print("Normalize Logistic Regression Accuracy:", accuracy(y_test, y_pred))
